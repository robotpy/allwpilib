...
[[meta-client-sub]]
==== Client Subscriptions (`$clientsub$<topic>`)
...
The server shall send this message to all clients upon initial connection for each existing topic, when a topic is created, or when any topic flags are changed.  Additionally the server shall send this message to a client in response to an `publish` message.
